...
public:
	...
	virtual bool lockSurface(JNIEnv *env, EmuEngine::Surface *info, bool flip);
	virtual void unlockSurface(JNIEnv *env);

	virtual bool audioCreate(JNIEnv *env, unsigned int rate, int bits, int channels);
...
bool EmuMediaImpl::init(JNIEnv *env)
{
	...
	midSetSurface = env->GetStaticMethodID(jPeerClass, "setSurface", "(Landroid/view/SurfaceHolder;)V");
	midSetSurfaceRegion = env->GetStaticMethodID(jPeerClass, "setSurfaceRegion", "(IIII)V");
	midBitBlt = env->GetStaticMethodID(jPeerClass, "bitBlt", "([IZ)V");
	midAudioCreate = env->GetStaticMethodID(jPeerClass, "audioCreate", "(III)Z");
	midAudioSetVolume = env->GetStaticMethodID(jPeerClass, "audioSetVolume", "(I)V");
